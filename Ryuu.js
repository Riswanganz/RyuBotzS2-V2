...
case 'sticker': case 'stikerin':
				case 'stiker': case 's':{
						...
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							const media = await Ryuu.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
                        ...
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', async function () {
								...
								await Ryuu.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: fgif})
								fs.unlinkSync(media)	
                    ...
                    //    ran = getRandom('.webp')
                        ...
                        await removeBackgroundFromImageFile({ path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp }).then(res => {
                            ...
                            exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
                                ...
                                Ryuu.sendMessage(from, fs.readFileSync(ranw), sticker, { quoted: mek })
                                    fs.unlinkSync(ranw)
                                })
                            })
                    } else {
